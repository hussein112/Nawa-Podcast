...
const Podcast = ({ podcast, id, wpId, media, togglePlaying }) => {
    ...
    const handleConvert = (dateString) => {
        const dateObj = new Date(dateString);

        const year = dateObj.getFullYear();
        const month = String(dateObj.getMonth() + 1).padStart(2, '0'); // Pad month with leading zero
        const day = String(dateObj.getDate()).padStart(2, '0'); // Pad day with leading zero
  ...
  return (
    <div className={"post post-single " + id + (media === "video" ? " border" : "")}>
        {media === "audio" ? <img src={podcast._embedded['wp:featuredmedia']['0'].source_url} alt="a Post" className='bg' /> :
        ...
        <div className="container" style={{opacity: opacity}}>
            ...
            <h2>بودكاست</h2>
            <a style={{cursor: 'pointer'}} onClick={() => setShowPopUp([true, podcast])} dangerouslySetInnerHTML={{__html: podcast.title.rendered}}>{}</a>
            <span className='date'>{handleConvert(podcast.date)}</span>
        </div>
        {media === "audio" ? 
